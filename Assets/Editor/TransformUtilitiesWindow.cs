...
/// <summary>
/// 实现了GameObject的空间对齐和拷贝（也就是将GameObject A作为基准，选中其他的GameObject进行对准或空间位置拷贝），对齐和拷贝提高
/// 了开发者摆放物件的效率；另外还有随机和噪声，后两者用于摆放大量同类物件的时候可以使用，比如一大堆散落的瓶子。
/// </summary>
public class TransformUtilitiesWindow : EditorWindow {
    ...
    private void CreateAlignTransformWindow()
    {
        ...
        source = EditorGUILayout.ObjectField(source , typeof(Transform), allowSceneObjects:false) as Transform;
        GUILayout.EndHorizontal();
